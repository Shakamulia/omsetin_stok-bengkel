...
class _ReportProfitState extends State<ReportProfit> {
  ...
  Future<List<Map<String, dynamic>>> _loadReportData() async {
    try {
      ...
      final totalModal = _calculateTotalModal(filteredTransactions);
  ...
  int _calculateTotalModal(List<TransactionData> filteredTransactions) {
    int totalModal = 0;

    for (var transaction in filteredTransactions) {
      for (var product in transaction.transactionProduct) {
        int purchasePrice = product['product_purchase_price'] ?? 0;
        int quantity = product['quantity'] ?? 0;
        totalModal += purchasePrice * quantity;
      }
    }
    return totalModal;
  }
